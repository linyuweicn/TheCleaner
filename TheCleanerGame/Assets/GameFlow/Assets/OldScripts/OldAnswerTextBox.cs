...
public class OldAnswerTextBox : MonoBehaviour, IPointerClickHandler //handles behavior of AnswerTextBox
{
    ...
    [SerializeField] OldAnswerTextBoxVisuals ATVisuals;
    [SerializeField] LayerMask answerTextLayer;
    [HideInInspector] public OldAnswerManager answerManager;
    Camera cam;
    OldAnswerTextBox lastSwapped;
    ...
    {
        cam = Camera.main;
        answerManager = OldGeneralFlowStateManager.instance.answerManager;
    ...
    public void Construct(OldPrompt prompt, OldAnswer answer, int ranking)
    {
        this.prompt = prompt;
        this.answer = answer;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (answerTextLayer == (answerTextLayer | (1 << collision.gameObject.layer)))
        {
            ...
            if (mouseClickedOn)
            {
                OldAnswerTextBox other = collision.gameObject.GetComponent<OldAnswerTextBox>();
    ...
    public void SwapRankingsAmongAnswers(OldAnswerTypes type, int first, int second) //swaps interpersonal rankings (in answer)
    {
        OldAnswerTextBox tempAnswer = answerManager.generatedAnswers[type][first];
    ...
    public void SwapPositionsAmongAnswers(OldAnswerTypes type, int first, int second)
    {
        if (first != second)
        {
            ...
            OldGeneralFlowStateManager.instance.VisitPrompt();
        }
