...
public class SafetyBar : MonoBehaviour
{
	...
	[Range(0, 1f)] public float m_SafetyFillAmount = 0.0f;
	...
	private void Start()
	{
		m_SafetyFillAmount = Score.SafetyValue / 100;
	}
	private void Update()
	{
		for (int i = 0; i < m_NumberOfSegments; i++)
		{
			m_SafetyFillAmount = Score.SafetyValue / 100;
			m_ProgressToFill[i].fillAmount = (m_SafetyFillAmount * ((m_EndAngle - m_StartAngle) / 360)) - m_SizeOfSegment * i;
		}
	}
