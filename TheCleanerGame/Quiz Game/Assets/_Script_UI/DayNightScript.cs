...
public class DayNightScript : MonoBehaviour
{
    ...
    {
        //ppv.weight = 0;
        if (hours >= 20 && hours < 21) // dusk at 21:00 / 9pm    -   until 22:00 / 10pm
        {
            ...
            for (int i = 0; i < stars.Length; i++)
            {
                ...
                //stars[i].DOColor( new Color (59f,72f,77f,255f),2f);
            }

            if (activateLights == false) // if lights havent been turned on
            {
                if (mins > 40) // wait until pretty dark
                {
                    for (int i = 0; i < lights.Length; i++)
                    {
    ...
    public void DisplayTime() // Shows time and day in ui
    {
        ...
        timeDisplay.text = string.Format("{0:00}:{1:00}", hours, 0); // The formatting ensures that there will always be 0's in empty spaces
