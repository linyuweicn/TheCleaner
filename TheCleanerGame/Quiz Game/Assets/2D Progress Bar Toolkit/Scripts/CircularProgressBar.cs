...
public class CircularProgressBar : MonoBehaviour {
	...
	[Range(0, 1f)] public float m_PSFillAmount = 0.0f;
	...
	private void Start()
    {
		m_PSFillAmount = Score.PlayerSatisValue / 100;

	}
	private void Update() {
		for (int i = 0; i < m_NumberOfSegments; i++) {
			m_PSFillAmount = Score.PlayerSatisValue / 100;
			m_ProgressToFill [i].fillAmount = (m_PSFillAmount * ((m_EndAngle-m_StartAngle)/360)) - m_SizeOfSegment * i;
		}
	}
